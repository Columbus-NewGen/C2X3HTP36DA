...
// ในระดับ Production เราจะเก็บข้อมูลแยกเป็น Stage เพื่อให้ง่ายต่อการ Scale
...
export default function StreakPetCard({ streakDays = 0 }: { streakDays: number }) {
  ...
  return (
    <div className="flex items-center justify-center w-full font-sans">
      <motion.div
        layout
        className="relative w-full max-w-sm p-1 rounded-[2.5rem] overflow-hidden"
        style={{ background: `linear-gradient(180deg, ${stage.color}44 0%, #1A1A1C 100%)` }}
      >
        {/* Main Card Body */}
        <div className="relative bg-[#111112] rounded-[2.4rem] p-8 overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-80 blur-[100px] pointer-events-none"
          ...
          <div className="relative z-10 flex justify-between items-start mb-12">
            <div>
              <p className="text-[10px] font-bold  uppercase text-white/30 mb-1">Current Status</p>
              <h2 className="text-xl font-bold  text-white">{stage.label}</h2>
            </div>
            <div className="px-3 py-1 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
              <span className="text-[10px] font-bold text-white/60  uppercase">Level {EVOLUTION_STAGES.indexOf(stage) + 1}</span>
            </div>
          </div>
          ...
          <div className="relative h-56 flex items-center justify-center mb-10">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* The Sprite with floating effect */}
                <motion.img
                  animate={{
                    y: [0, -15, 0],
                    rotate: [0, 2, -2, 0]
                  ...
                  src={stage.image}
                  alt={stage.title}
                  className="w-44 h-44 object-contain relative z-10 drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
                />
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="relative z-10 text-center space-y-4">
            <div>
              <motion.h1
                ...
                className="text-6xl font-bold italic  text-white"
              >
                {streakDays}
                <span className="text-sm not-italic font-bold  text-white/20 ml-3">DAYS</span>
              </motion.h1>
              <motion.p
